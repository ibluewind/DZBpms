...
<body ng-app="bpmApp" ng-cloak>
	<div class="container-fluid" ng-controller="listController as ctrl">
				...
				<table class="table table-hover">
					...
					<tbody>
						<tr ng-repeat="form in ctrl.forms">
							<td>{{form.title}}</td>
							<td>{{form.comment}}</td>
							<td>{{form.created|date: 'yyyy-MM-dd HH:mm:ss'}}</td>
							<td>{{form.modified|date: 'yyyy-MM-dd HH:mm:ss'}}</td>
							<td>
								<button type="button" class="btn btn-primary btn-sm" ng-click="ctrl.edit(form.id)">Edit</button>
								<button type="button" class="btn btn-danger btn-sm" ng-click="ctrl.remove(form.id)">Delete</button>
							</td>
						</tr>
					</tbody>
				</table>
