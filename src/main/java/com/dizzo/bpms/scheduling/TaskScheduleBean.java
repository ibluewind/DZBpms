...
public class TaskScheduleBean {
	...
	public void checkTargetRate() {
		 ...
		 */
		String query = "update task set targetRate=round((datediff(enddate, createDate) + 1 - datediff(enddate, now() + 1)) / (datediff(enddate, createDate) + 1) * 100, 0)";
		new JdbcTemplate(dataSource).update(query);
		
		// 진행중인 작업 중에서 현재 완료율이 목표 완료율에 미치지 않는 작업들은 모두 지연으로 표시한다.
		query = "update task set status='L' where currentRate < targetRate and status='P'";
		new JdbcTemplate(dataSource).update(query);
	}
}
